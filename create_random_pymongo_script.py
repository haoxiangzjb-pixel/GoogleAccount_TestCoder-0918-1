import pymongo
import random
import string
import os

def create_random_filename(ext='.py'):
    """Generate a random filename with the given extension."""
    random_part = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
    return f"pymongo_script_{random_part}{ext}"

def create_pymongo_insert_script():
    """Create a PyMongo script that inserts a document."""
    script_content = '''import pymongo
import random
import string

# Connect to MongoDB (assuming MongoDB is running locally on default port 27017)
client = pymongo.MongoClient("mongodb://localhost:27017/")

# Create a random database name
db_name = "test_db_" + ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
db = client[db_name]

# Create a random collection name
collection_name = "test_collection_" + ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))
collection = db[collection_name]

# Create a document to insert with random elements
document = {
    "name": "John Doe",
    "age": 30,
    "email": f"john.doe.{random.randint(1000, 9999)}@example.com",
    "random_id": ''.join(random.choices(string.ascii_letters + string.digits, k=10)),
    "timestamp": "2023-01-01T00:00:00Z"
}

# Insert the document
result = collection.insert_one(document)

print(f"Document inserted with ID: {result.inserted_id}")
print(f"Database: {db_name}")
print(f"Collection: {collection_name}")
print(f"Inserted document: {document}")

# Close the connection
client.close()
'''
    return script_content

# Generate a random filename
random_filename = create_random_filename()

# Get the PyMongo script content
script_content = create_pymongo_insert_script()

# Write the script to the randomly named file
with open(random_filename, 'w') as file:
    file.write(script_content)

print(f"PyMongo script saved to: {random_filename}")
print("Script content:")
print(script_content)